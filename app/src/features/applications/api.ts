...
// Firestore에 저장되는 문서 타입 (id 제외)
type JobApplicationDoc = Omit<JobApplication, "id">;
...
function mapApplicationDoc(
  snap: QueryDocumentSnapshot<JobApplicationDoc> | DocumentSnapshot<JobApplicationDoc>,
): JobApplication {
  const data = snap.data() as JobApplicationDoc;
  return {
    id: snap.id,
    ...data,
  };
...
export async function createApplication(
  ...
  return mapApplicationDoc(snap as DocumentSnapshot<JobApplicationDoc>);
}
...
export async function listApplications(
  ...
  return snap.docs.map((docSnap) =>
    mapApplicationDoc(
      docSnap as QueryDocumentSnapshot<JobApplicationDoc>,
    ),
  );
}
